...
# ## **Drawing CHADA tables as a plantuml json diagram from a single json file**

# In this example we will see how to use convenience functions in `mochada_kit/tables` to help us draw plantuml json diagrams showing CHADA tables from a single json file as input.
...
# 
# **OK, so why should we draw CHADA tables with plantuml from json?**
# 
# 1. json is a very simple [file format](https://en.wikipedia.org/wiki/JSON) which is arguably easier to parse than .odt or .docx. This may simplify the interaction with ontology development.
...
#   
# Let's have a look at these options. First, we import the necessary modules:
...
# **2a Templates**
# 
# The folder `mochada_kit/templates` contains json files as templates for the CHADA tables. The file `chada_tables_template.json` contains all the headings in the tables but only empty strings for the content. The file `chada_tables_template_tips.json` contains all the headings in the tables and for each heading there is some text giving tips about how to fill in the content. Let's see how to plot all the tables in this file in one json diagram.
...
# 
# - the first argument (`json_path`) must be the path to the json file containing the data to be plotted. Either a string or a pathlib.Path
# - to save the plantuml code file in a *different* folder to the json file, we need to supply the argument `out_path`, which points to the folder where we want to store the plantuml code either as an absolute or relative path (either as a string or a pathlib.Path). If a relative path is supplied, it is assumed to be relative to the parent folder of json_path.
# - we have two options:
...
# 
# The various options are summarised in the table below:
# 
...
# |:---|:---|:---|
# |json_path | full path to .json | path to .json relative to the current working directory |
# |out_path | full path to the output puml code (.puml) | path to output .puml relative to the parent folder of json_path|
# |load_path | full path to .json | path to .json relative to out_path |
...
json_path = pathlib.Path("../templates/chada_tables_template_tips.json") # path to the json relative to the current working directory

out_path = pathlib.Path("../gallery/puml_code") # path to the folder where the output puml code is stored (relative to the parent folder of json_path)

load_path = pathlib.Path("../../templates/chada_tables_template_tips.json") # relative path from out_path to json_path
...
# **2c Names**
# 
# - By default, the name of the puml code output file will be the same as that of the json input file.
...
mt.write_chada_tables_whole_plantuml(json_path, out_path=out_path, load_path=load_path, out_name=out_name, theme_name="plasma_wide")
...
out_base = mt.handle_paths(json_path,
                           out_path=out_path,
                           load_path=load_path,
                           out_name=out_name,
...
mt.write_chada_tables_plantuml(json_path, out_path=out_path, load_path=load_path, out_name=out_name_2)


# In[9]:


out_base_2 = mt.handle_paths(json_path,
                             out_path=out_path,
                             load_path=load_path,
                             out_name=out_name_2,
                             return_out_base_only=True
...
# This time, we used the standard `plasma` theme, rather than `plasma_wide` and therefore the json elements appear narrower than in the first diagram. We can see tha the "contents" element contains hyperlinks. These don't work when embedded in a jupyter notebook but if you open the .svg file in a browser, you can use the links to navigate between the various tables.
...
mt.write_chada_tables_single_plantuml(json_path, out_path=out_path, load_path=load_path, out_name=out_name_3, theme_name="plasma_A4w")


# In[13]:


out_base_3 = mt.handle_paths(json_path,
                             out_path=out_path,
                             load_path=load_path,
                             out_name=out_name_3,
                             return_out_base_only=True
...
# <br>
# 
# #### **5. Advanced formatting of the json data**

# To round off this example notebook, let's have a look at how we can add some advanced formatting to the json data, rather than just text.
# 
# Even though `mochada_kit/data/demo_formatting.json` has the correct format for CHADA tables, instead of using the functions to generate all the CHADA tables, we'll just write the puml code for one table in this case. The code below results in a contents element joined to the "experiment" element, which contains the advanced formatting. Further explanations for this diagram are given below.
...
with pathlib.Path("../gallery/puml_code/demo_formatting.puml") as f:
...
# 
# **Note that escape characters *e.g.* "\\n" must be double escaped "\\\\\n" in the json file in order to have an effect in the final diagram**
# 
